...
import { DynamicQueryBuilder } from "CMS-BACK-END/src/core/repository/search/query-builder";
...
export class UserDbRepository implements UserRepository {
	...
	async findAllLabelValuePairByIds(list: string[]): Promise<LabelValuePair[]> {
		await this.setRepository();
		const query = `SELECT id as value, user_name as "userName",employee_id as "employeeId" from ${
			this.repository.schema
