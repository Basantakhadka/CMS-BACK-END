...
export class FilterCondition extends EnumType<FilterCondition> {
	...
	private prepareDynamicCriteria(filters: Filter) {
		...
		if (this.name == FilterCondition.BETWEEN.name) {
			...
			filterExpression += ` ${ FilterCondition.GREATER_THAN_OR_EQUALS.operator
				} ${ this.checkDateAndTimestampType(
					dataType
				) } ${ this.buildFilterValuesExpression([values[0]]) } ${ this.valueSeperator
				} ${ field } ${ FilterCondition.LESS_THAN_OR_EQUALS.operator
				} ${ this.checkDateAndTimestampType(
					dataType
		...
		filterExpression += ` ${ field } ${ this.operator } ${ this.enclosedByParenthesis
			? ` (${ this.checkDateAndTimestampType(
				dataType
